...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
	...
	public GamePanel() {
		...
		s = new Ship(Main.width / 2 - Ship.sideLength / 2, Main.height / 2 + Ship.sideLength / 2);
		...
		for (int i = 0; i < enemiesAmount; i++) {
			...
			if (top)
				...
				ems.add(new Enemy((int) (Math.random() * Main.width - 9) + 10, (int) (Math.random() * 11) + 10, top,
						right, left, bottom));
			if (bottom)
				// System.out.println("bottom");
				ems.add(new Enemy((int) (Math.random() * Main.width - 9) + 10, (int) (Math.random() * 11) + Main.height - 70, top,
						right, left, bottom));
			if (left)
				// System.out.println("top");
				ems.add(new Enemy((int) (Math.random() * 11) + 5, (int) (Math.random() * Main.height - 9), top, right,
						left, bottom));
			if (right)
				// System.out.println("right");
				ems.add(new Enemy((int) (Math.random() * 11) + Main.width - 40, (int) (Math.random() * Main.height - 9), top,
						right, left, bottom));
		}
	...
	public void drawEndStage(Graphics g) {
		...
		g.drawString("Yikers, you lost, wanna restart?", Main.width/2 - 300, Main.height/2- 100);
		g.drawString("If so, then click enter", Main.width/2 - 200, Main.height/2);
	}
	...
	public void drawWinStage(Graphics g) {
		...
		if (System.currentTimeMillis() - winTime <= 5000) {	
			...
			g.drawString("Nice, you beat enough!", Main.width/2 - 250 , Main.height/2 - 50);
			g.drawString("You can now leave in peace", Main.width/2 - 280, Main.height/2);
		}
	}
	public void updateGameStage() {
		...
		if (gameOver) {
			s.setX(Main.width / 2 - Ship.sideLength / 2);
			s.setY(Main.height / 2 + Ship.sideLength / 2);
			p = new Projectile(s.getX(), s.getY());
			ems.clear();
			resetEnemies(ems);
			amountRemaining = ems.size();
	...
	public void updateWinStage() {
		...
		Stage4.gameFrame.setVisible(false);
	...
	public static void resetEnemies(ArrayList<Enemy> ems) {
		...
		for (int i = 0; i < enemiesAmount; i++) {
			...
			if (bottom)
				// System.out.println("bottom");
				ems.add(new Enemy((int) (Math.random() * Main.width - 9) + 10, (int) (Math.random() * 11) + 1010, top,
						right, left, bottom));
			if (left)
				// System.out.println("top");
				ems.add(new Enemy((int) (Math.random() * 11) + 5, (int) (Math.random() * Main.height - 9), top, right,
			...
			if (right)
				// System.out.println("right");
				ems.add(new Enemy((int) (Math.random() * 11) + 1880, (int) (Math.random() * Main.height - 9), top,
						right, left, bottom));
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (GAME_STAGE == 0) {
			...
			if (e.getKeyCode() == KeyEvent.VK_SPACE && nextShot == true) {
